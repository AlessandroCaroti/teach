...
try {
    Write-Step "Installing teach"

    if ($UseRemoteTeach) {
        $teachRepo = Join-Path $tempRoot "teach-repo"
        Clone-Repo -Url "https://github.com/AlessandroCaroti/teach.git" -Destination $teachRepo
        $TeachPath = Join-Path $teachRepo "skills\teach"
    }

    if (-not (Test-Path -LiteralPath (Join-Path $TeachPath "SKILL.md"))) {
        throw "teach skill was not found at $TeachPath"
    }

    Copy-Skill -Source $TeachPath -Name "teach"
